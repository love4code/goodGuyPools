...
exports.create = async (req, res) => {
  try {
    ...
    const slug = slugify(title || '', { lower: true, strict: true }) || undefined
    await Service.create({ title, description, icon: icon || '', order: Number(order) || 0, slug })
...
exports.update = async (req, res) => {
  try {
    const { title, description, order, icon } = req.body
    const update = { title, description, icon: icon || '', order: Number(order) || 0, updatedAt: new Date() }
    await Service.findByIdAndUpdate(req.params.id, update, { runValidators: true })
